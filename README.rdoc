...
Our Classes are defines by the session that every user have every time he connects to our service, it saves all the necessary data and uses 
...
views: here we have all our .erb files that we use
...
- answer quiz and not upload grade: the user will answer the quiz and won't upload the grade.
